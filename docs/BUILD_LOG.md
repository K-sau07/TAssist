...
**3.3 status:** structurally COMPLETE + tested. The only unverifiable-without-live-creds path is the real code→token exchange (callback happy path); needs a real Google authorization code. To activate: create OAuth client in Google Cloud Console (redirect URI `http://localhost:8080/api/auth/google/callback`, scopes openid/email/profile), drop GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET into `.env`. Code returns 503 on /authorize until creds present.
